...
// şifre/email kuralı ihlali) — web'deki aynı düzeltme burada da geçerli.
...
// Google/Apple ile giriş (2026-09-16) - backend iki farklı şekilde
// yanıtlayabiliyor (bkz. backend/app/schemas/user.py::OAuthResult): sub
// zaten bilinen bir kullanıcıya eşleşiyorsa `status: "logged_in"` +
// token'lar dolu gelir (normal login ile aynı); YENİ bir kullanıcıysa
// `status: "consent_required"` + `pending_token` gelir - hesap KVKK/sağlık
// verisi/Kullanım Koşulları rızası tamamlanana kadar açılmıyor (bkz.
// oauth-consent.tsx, completeOAuth).
...
export function getUnreadCheckinCount(token: string) {
  // list_checkins'in AKSİNE - hiçbir satırı okunmuş işaretlemez, rozet için
  // güvenle sık sık çağrılabilir (bkz. backend checkin_service.count_unread).
...
// tab'lar unmount olmadığı için mobile'da hemen fark edilen bir bug'dı).
