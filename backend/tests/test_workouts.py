...
def test_delete_session_endpoint_not_found(client):
    ...
    assert response.json()["detail"] == "Antrenman oturumu bulunamadı."


def test_delete_session_endpoint_not_found_respects_english_preference(client):
    """Regresyon: REST 404 mesajları artık preferred_language'e göre
    İngilizce dönebiliyor (2026-08-10 pürüz taraması, Tema C)."""
    headers = _register_and_login(client, email="workout-api-delete-404-en@example.com")
    client.patch("/profile", json={"preferred_language": "en"}, headers=headers)
    response = client.delete("/workouts/sessions/999999", headers=headers)
    assert response.status_code == 404
